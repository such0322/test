...
//var_dump($_SESSION["__mylog"]);
	...
	$default_limit  = 100;
	$default_offset = 0;
	$default_order  = 'l_shop_id';
	$default_display_cols = array('log_date', 'l_shop_id', 'user_id', 'chara_id', 'item_id', 'item_num', 'subtotal', );
...
//var_dump($sql);
		...
		if ($type == 'ext') {
			...
			// 購入者数
			$query = "SELECT count(distinct uid) AS c FROM {$table_name}";
			if (sizeof($where_array) > 0) {
