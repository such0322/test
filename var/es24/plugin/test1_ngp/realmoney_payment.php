...
//var_dump($_SESSION["__mylog"]);
	...
	$default_limit  = 100;
	$default_offset = 0;
	$default_order  = 'l_realmoney_payment_id';
	$default_display_cols = array('log_date', 'l_realmoney_payment_id', 'uid', 'user_id', 'trade_val', 'goods_id', );
	
	$table_name = 'l_realmoney_payment';
...
//mylog($sql);mylog($arr);
		...
		if ($type == 'goodsstat') {
			...
			$sql = "SELECT date_format({$stat_key}, '{$stat_units[$stat_unit]['df']}') AS d, sum(trade_val) AS gaku, goods_id FROM {$table_name} ";
			if (sizeof($where_array) > 0) {
				$sql .= ' WHERE ' . implode(' AND ', $where_array);
			}
			$sql .= ' GROUP BY d, goods_id ORDER BY d, goods_id';
			$arr = db_select($main_con, $sql);
//var_dump($sql);var_dump($post_data);var_dump($arr);
//mylog($sql);mylog($arr);
			...
			if ($arr) {
				...
				foreach ($arr AS $rec) {
					...
					while (strtotime($stat_units[$stat_unit]['range'], $last_ts) < $ts) {
						...
						foreach ($master['goods_id'] as $goods_id => $goods_name) {
							$a[$goods_id] = 0;
						}
						$records[$last_ts] = $a;
					}
					...
					$records[$last_ts][$rec['goods_id']] = $rec['gaku'];
				}
...
//mylog($sql);mylog($arr);
			...
			if ($arr) {
				...
				foreach ($arr AS $rec) {
					...
					$records[$last_ts][$goods_to_group[$rec['goods_id']]] += $rec['gaku'];
				}
