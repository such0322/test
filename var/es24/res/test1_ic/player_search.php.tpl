<script language="javascript">
	function f1_csv_export() {
		document.getElementById("f1").action = "inline.php?include_file=plugin/{{$menukey}}/player_search.php&menukey={{$menukey}}";
					...
					{{foreach name=select_columns key=k item=i from=$select_columns }}
						<tr class="tr{{cycle values="1,2"}}">
							...
							{{elseif $i.type == 'datetime' || $i.type == 'unixtime'}}
								<td>
									<input type="text" id="f1_{{$k|replace:".":"__"}}_begin" name="{{$k}}[begin]" value="{{if $post_data.$k.begin !== null}}{{$post_data.$k.begin|escape:"html"}}{{else}}{{/if}}" class="datetime" size="20" maxlength="20" />
									　～　
									<input type="text" id="f1_{{$k|replace:".":"__"}}_end" name="{{$k}}[end]" value="{{if $post_data.$k.end !== null}}{{$post_data.$k.end|escape:"html"}}{{else}}{{/if}}" class="datetime" size="20" maxlength="20" />
									{{if $is_change_tz}}
...
$(document).ready(function(){
	...
	$("#f1_where_update").click(function(){
		...
		var f1_postdata = [
			...
			, {"name":"include_file","value":"plugin/{{$menukey}}/player_search.php"}
		];
		...
		$.get("inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/player_search.php", f1_postdata, function(data){
			// 検索条件を変えたら表を再読み込み
	...
	$('#f2_history').flexigrid(
		{
			url: 'inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/player_search.php&json=1&type=history',
			title: 'キャラ検索', 
			nomsg: 'not found', 
			colModel :
			[
			...
			,preProcess: function(data){
				$.each(data.rows, function(i, val){
					val.cell[0] = '<a href="?menukey={{$menukey}}&include_file={{$menukey}}/player_detail.php&f1_submit=1&player_id=' + val.cell[0] + '">' + val.cell[0] + '</a>';
				});
				return data;
			}
		}
