...
<!--
	function f1_csv_export() {
		document.getElementById("f1").action = "inline.php?include_file=plugin/{{$menukey}}/shop.php&menukey={{$menukey}}";
...
$(document).ready(function(){
	...
	$("#f1_where_update").click(function(){
		...
		f1_postdata = [
			...
			, {"name":"include_file","value":"plugin/{{$menukey}}/shop.php"}
		];
		...
		$.get("inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/gacha.php", f1_postdata, function(data){
			...
			$.ajax({
				url: "inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/shop.php&json=1&type=ext",
				dataType: 'json',
				data: {},
	...
	$('#f2_history').flexigrid(
		{
			url: 'inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/shop.php&json=1&type=history',
			title: '{{$pstrres.sh01}}', 
			nomsg: '{{$pstrres.sh02}}', 
			colModel :
			[
	...
	$('#f2_ranking').flexigrid(
		{
			url: 'inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/shop.php&json=1&type=ranking',
			title: '{{$pstrres.sr01}}', 
			nomsg: '{{$pstrres.sr02}}', 
			colModel :
			[
				  {display: '',                  name : 'name',  width : 200, sortable : false, align: 'left'}
				, {display: '{{$pstrres.sr04}}', name : 'count', width : 120, sortable : false, align: 'right'}
				, {display: '{{$pstrres.sr05}}', name : 'sum',   width : 120, sortable : false, align: 'right'}
			],
			autoload: false, 
	...
	$('#f2_stat').flexigrid(
		{
			url: 'inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/shop.php&json=1&type=stat',
			title: '{{$pstrres.ss01}}', 
			nomsg: '{{$pstrres.sks02}}', 
			colModel :
			[
				  {display: '{{$pstrres.sr03}}', name : 'log_type', width : 200, sortable : false, align: 'left'}
				, {display: '{{$pstrres.sr04}}', name : 'sales',    width : 120, sortable : false, align: 'right'}
				, {display: '{{$pstrres.sr05}}', name : 'subtotal', width : 120, sortable : false, align: 'right'}
			],
			autoload: false, 
...
<div align="left" style="white-space: nowrap;">
	<div align="right">
		{{$pstrres.f2001}}：<span id="f2_buy_user_count">{{$buy_user_count}} 人</span><br>
		{{$pstrres.f2002}}：<span id="f2_sales">{{$salecount}}</span><br>
		{{$pstrres.f2003}}：<span id="f2_salecount">{{$sales}}</span><br>
