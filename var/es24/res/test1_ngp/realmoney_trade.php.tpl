...
<!--
	function f1_csv_export() {
		document.getElementById("f1").action = "inline.php?include_file=plugin/{{$menukey}}/realmoney_trade.php&menukey={{$menukey}}";
...
$(document).ready(function(){
	...
	$("#f1_where_update").click(function(){
		...
		f1_postdata = [
			...
			, {"name":"include_file","value":"plugin/{{$menukey}}/realmoney_trade.php"}
		];
		...
		$.get("inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/realmoney_trade.php", f1_postdata, function(data){
	...
	$('#f2_history').flexigrid(
		{
			url: 'inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/realmoney_trade.php&json=1&type=history',
			title: '{{$pstrres.kh01}}', 
			nomsg: '{{$strres.lnf}}', 
			colModel :
			[
	...
	$('#f2_ranking').flexigrid(
		{
			url: 'inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/realmoney_trade.php&json=1&type=ranking',
			title: '{{$pstrres.gs01}}', 
			nomsg: '{{$strres.lnf}}', 
			colModel :
			[
				  {display: '{{$gr01}}', name : 'goods', width : 200, sortable : true, align: 'left'}
				, {display: '{{$gr02}}', name : 'count', width :  80, sortable : true, align: 'right'}
				, {display: '{{$gr03}}', name : 'sum',   width :  80, sortable : true, align: 'right'}
			],
			autoload: false, 
	...
	$('#f2_stat').flexigrid(
		{
			url: 'inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/realmoney_trade.php&json=1&type=stat',
			title: '{{$pstrres.gs01}}', 
			nomsg: '{{$strres.lnf}}', 
			colModel :
			[
				...
				, {display: '{{$pstrres.gs06}}', name : 'sum',  width :  80, sortable : false, align: 'right'}
			],
			autoload: false, 
...
<div align="left" style="white-space: nowrap;">
	<div id="tabs">
		<ul>
			...
			<li><a href="#tabs-2">{{$pstrres.l003}}</a></li>
