...
<!--
	function f1_csv_export() {
		document.getElementById("f1").action = "inline.php?include_file=plugin/{{$menukey}}/freemoney.php&menukey={{$menukey}}";
...
$(document).ready(function(){
	...
	$("#f1_where_update").click(function(){
		...
		f1_postdata = [
			...
			, {"name":"include_file","value":"plugin/{{$menukey}}/freemoney.php"}
		];
		...
		$.get("inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/freemoney.php", f1_postdata, function(data){
...
{{*
			$.ajax({
				url: "inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/freemoney.php&json=1&type=ext",
				dataType: 'json',
				data: {},
...
*}}
	...
	$('#f2_history').flexigrid(
		{
			url: 'inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/freemoney.php&json=1&type=history',
			title: '{{$pstrres.kh01}}', 
			nomsg: '{{$strres.lnf}}', 
			colModel :
			[
	...
	$('#f2_ranking').flexigrid(
		{
			url: 'inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/freemoney.php&json=1&type=ranking',
			title: '{{$pstrres.kr01}}', 
			nomsg: '{{$strres.lnf}}', 
			colModel :
			[
				  {display: '',                  name : 'name', width : 200, sortable : false, align: 'left'}
				, {display: '{{$pstrres.kr04}}', name : 'add',  width : 120, sortable : false, align: 'right'}
				, {display: '{{$pstrres.kr05}}', name : 'sub',  width : 120, sortable : false, align: 'right'}
				, {display: '{{$pstrres.kr06}}', name : 'lost', width : 120, sortable : false, align: 'right'}
			],
			autoload: false, 
	...
	$('#f2_stat').flexigrid(
		{
			url: 'inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/freemoney.php&json=1&type=stat',
			title: '{{$pstrres.gs01}}', 
			nomsg: '{{$strres.lnf}}', 
			colModel :
			[
				  {display: '{{$pstrres.gs02}}', name : 'date', width : 200, sortable : false, align: 'left'}
				, {display: '{{$pstrres.gs03}}', name : 'buy',  width :  80, sortable : false, align: 'right'}
...
*}}
	<div id="tabs">
		<ul>
			...
			<li><a href="#tabs-2">{{$pstrres.kr01}}</a></li>
