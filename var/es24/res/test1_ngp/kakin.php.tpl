...
<!--
	function f1_csv_export() {
		document.getElementById("f1").action = "inline.php?include_file=plugin/{{$menukey}}/kakin.php&menukey={{$menukey}}";
				...
				<tbody>
					{{foreach name=select_columns key=k item=i from=$select_columns }}
						<tr class="tr{{cycle values="1,2"}}">
							...
							{{elseif $i.type == 'subquery'}}
								<td>
									<input type="checkbox" name="{{$k}}" value="1" {{if $post_data.$k}}checked{{/if}}>
								</td>
							{{else}}
								<td>&nbsp;</td>
							{{/if}}
...
$(document).ready(function(){
	...
	$("#f1_where_update").click(function(){
		...
		var f1_postdata = [
			...
			, {"name":"include_file","value":"plugin/{{$menukey}}/kakin.php"}
		];
		...
		$.get("inline.php", f1_postdata, function(data){
			...
			if ($('#tabs-2b').is(':visible')) {$('#f2_granking').flexReload();}
			if ($('#tabs-3').is(':visible')) {$('#f2_stat').flexReload();}
			
			$.ajax({
				url: "inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/kakin.php&json=1&type=ext",
				dataType: 'json',
				data: {},
				success: function(data) {
					$('#f2_buy_user_count').text(data.buy_user_count);
					$('#f2_sales').text(data.sales);
					$('#f2_salecount').text(data.salecount);
					$('#f2_avg_user_buy').text(data.avg_user_buy);
				}
			});
	...
	$('#f2_history').flexigrid(
		{
			url: 'inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/kakin.php&json=1&type=history',
			title: '{{$pstrres.kh01}}', 
			nomsg: '{{$pstrres.kh02}}', 
			colModel :
			[
	...
	$('#f2_ranking').flexigrid(
		{
			url: 'inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/kakin.php&json=1&type=ranking',
			title: '{{$pstrres.kr01}}', 
			nomsg: '{{$pstrres.kr02}}', 
			colModel :
			[
				  {display: '',                  name : 'name',  width : 200, sortable : false, align: 'left'}
				, {display: '{{$pstrres.kr04}}', name : 'count', width : 120, sortable : false, align: 'right'}
				, {display: '{{$pstrres.kr05}}', name : 'sum',   width : 120, sortable : false, align: 'right'}
			],
			autoload: false, 
...
{{if $is_grouped_item_id}}
	$('#f2_granking').flexigrid(
		{
			url: 'inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/kakin.php&json=1&type=granking',
			title: '{{$pstrres.kr01}}', 
			nomsg: '{{$pstrres.kr02}}', 
			colModel :
			[
				  {display: '',                  name : 'name', width : 200, sortable : false, align: 'left'}
				, {display: '{{$pstrres.kr04}}', name : 'kazu', width : 120, sortable : false, align: 'right'}
				, {display: '{{$pstrres.kr05}}', name : 'gaku', width : 120, sortable : false, align: 'right'}
			],
			autoload: false, 
...
{{/if}}
	...
	$('#f2_stat').flexigrid(
		{
			url: 'inline.php?menukey={{$menukey}}&include_file=plugin/{{$menukey}}/kakin.php&json=1&type=stat',
			title: '{{$pstrres.ks01}}', 
			nomsg: '{{$pstrres.ks02}}', 
			colModel :
			[
				  {display: '{{$pstrres.kr03}}', name : 'log_type', width : 200, sortable : false, align: 'left'}
				, {display: '{{$pstrres.kr04}}', name : 'sales',    width : 120, sortable : false, align: 'right'}
				, {display: '{{$pstrres.kr05}}', name : 'subtotal', width : 120, sortable : false, align: 'right'}
			],
			autoload: false, 
...
<div align="left" style="white-space: nowrap;">
	<div align="right">
		{{$pstrres.f2001}}：<span id="f2_buy_user_count">{{$buy_user_count}}</span><br>
		{{$pstrres.f2002}}：<span id="f2_salecount">{{$salecount}}</span><br>
		{{$pstrres.f2003}}：<span id="f2_sales">{{$sales}}</span><br>
		{{$pstrres.f2004}}：<span id="f2_avg_user_buy">{{$avg_user_buy}}</span><br>
	</div>
	<div id="tabs">
		<ul>
...
{{if $is_grouped_item_id}}
			<li><a href="#tabs-2b">{{$strres.l003}} (group)</a></li>
{{/if}}
		</ul>
		<div id="tabs-1">
			<table id="f2_history">
